const translations = {
    ru: {
        ...
        hero_subtitle: "Индивидуальные туры и комфортабельный трансфер в любую точку страны.",
        ...
        feature_1_desc: "Все наши водители проходят строгий отбор, а автомобили регулярный техосмотр для вашего спокойствия.",
        feature_2_title: "Пунктуальность",
        feature_2_desc: "Мы ценим ваше время: подаем машину заранее и отслеживаем рейсы, чтобы встретить вас вовремя.",
        feature_3_title: "Честные цены",
        feature_3_desc: "Фиксированные цены без скрытых доплат. Вы знаете точную стоимость поездки еще до бронирования.",
        contact_title: "Готовы к поездке?",
        contact_desc: "Свяжитесь с нами любым удобным способом, и мы организуем вашу поездку на высшем уровне.",
    ...
    en: {
        ...
        hero_subtitle: "Individual tours and comfortable transfers to anywhere in the country.",
        ...
        feature_1_desc: "All our drivers undergo strict selection, and cars undergo regular technical inspection for your peace of mind.",
        feature_2_title: "Punctuality",
        feature_2_desc: "We value your time: we provide the car in advance and track flights to meet you on time.",
        feature_3_title: "Fair Prices",
        feature_3_desc: "Fixed prices with no hidden fees. You know the exact cost of the trip before booking.",
        contact_title: "Ready for a trip?",
        contact_desc: "Contact us in any convenient way, and we will organize your trip at the highest level.",
    ...
    const checkAndShowHint = (containerId, hintId, storageKey) => {
        ...
        setTimeout(() => {
            if (container.scrollWidth > container.clientWidth) {
                ...
                container.addEventListener('touchstart', onScroll, { passive: true, once: true });
            }
        }, 1000); // Delay to allow rendering and AOS animations
    };

    checkAndShowHint('routes-container', 'routes-hint', 'tvoitransfer_routes_hint_dismissed');
    checkAndShowHint('cars-container', 'cars-hint', 'tvoitransfer_cars_hint_dismissed');
}
...
function renderRoutes() {
    ...
    container.innerHTML = routes.map((route, index) => {
        const from = currentLang === 'en' && route.from_en ? route.from_en : route.from;
        const to = currentLang === 'en' && route.to_en ? route.to_en : route.to;

        return `
        <div class="min-w-[85vw] md:min-w-[380px] snap-center bg-white rounded-2xl shadow-xl overflow-hidden card-hover group border-2 border-transparent hover:border-primary/20 transition-all duration-300 flex flex-col" data-aos="${index % 2 === 0 ? 'fade-right' : 'fade-left'}" data-aos-delay="${index * 100}">
            <!-- Images Container -->
            <div class="relative h-56 overflow-hidden bg-white shrink-0">
                <!-- Image From (Left) -->
                <div class="absolute inset-0 w-full h-full" style="clip-path: polygon(0 0, 55% 0, 45% 100%, 0 100%);">
                    <img src="${route.image_from || route.image}" alt="${from}" class="w-full h-full object-cover transition duration-700 group-hover:scale-110">
                    <div class="absolute inset-0 bg-black/10 group-hover:bg-black/0 transition-colors duration-500"></div>
                </div>

                <!-- Image To (Right) -->
                <div class="absolute inset-0 w-full h-full" style="clip-path: polygon(56% 0, 100% 0, 100% 100%, 46% 100%);">
                    <img src="${route.image_to || route.image}" alt="${to}" class="w-full h-full object-cover transition duration-700 group-hover:scale-110">
                    <div class="absolute inset-0 bg-black/10 group-hover:bg-black/0 transition-colors duration-500"></div>
                    
                    <div class="absolute top-2 right-2 z-20">
                        <span class="bg-yellow-500 text-white text-[9px] font-extrabold px-2 py-0.5 rounded-full shadow-lg uppercase tracking-widest">${t.badge_transfer}</span>
                    </div>
                </div>

                <!-- Center Arrow Circle -->
                <div class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-30">
                    <div class="w-12 h-12 bg-white rounded-full flex items-center justify-center shadow-xl border-4 border-white text-gray-400 text-lg group-hover:rotate-180 transition-transform duration-500 group-hover:scale-110 group-hover:text-primary">
...
function renderReviews() {
    ...
    container.innerHTML = reviews.map((review, index) => {
        const date = new Date(review.date).toLocaleDateString(currentLang === 'ru' ? 'ru-RU' : 'en-US');
        const defaultAvatar = 'assets/images/default-avatar.png';
        const avatar = review.photo || `https://ui-avatars.com/api/?name=${encodeURIComponent(review.name)}&background=random`;

        return `
        <div class="min-w-[85vw] md:min-w-[350px] snap-center bg-white/80 backdrop-blur-md p-6 rounded-2xl shadow-lg border border-white/20" data-aos="fade-up" data-aos-delay="${index * 100}">
            <div class="flex items-center gap-4 mb-4">
                <img src="${avatar}" alt="${review.name}" class="w-12 h-12 rounded-full object-cover shadow-sm">
                <div>
...
function initReviewPage() {
    ...
    const updateFormText = () => {
        ...
        const photoLabel = document.querySelector('[data-i18n="review_photo_label"]');
...
function renderCars() {
    ...
    container.innerHTML = cars.map((car, index) => {
        ...
        if (currentLang === 'en' && car.features_en && car.features_en.length > 0) {
            features = car.features_en;
        }

        return `
        <div class="min-w-[85vw] md:min-w-[350px] snap-center bg-white rounded-xl shadow-lg overflow-hidden card-hover" data-aos="${index % 2 === 0 ? 'fade-right' : 'fade-left'}" data-aos-delay="${index * 100}">
            ...
            <div class="p-6">
                ...
                <div class="mt-4 pt-4 border-t border-gray-100">
                    ...
                    `).join('') : t.default_features.map(feature => `
                        <div class="flex items-center gap-2 text-sm text-gray-600 mb-2">
                            <i class="fas fa-check text-green-500"></i> ${feature}
                        </div>
                    `).join('')}
